(function(root, factory) {
  ...
  /* jshint maxstatements: 43, maxlen: 132, maxcomplexity: 8, latedef: false */
  ...
  // Tell Syphon to always include these extra types
  Syphon.extraFields = [];
  ...
  var getInputElements = function(view, config) {
    ...
    elements = _.reject(elements, function(el) {
      ...
      _.each(config.ignoredDatas, function(data) {
        if ($(el).data(data)) {
          reject = true;
        }
      });
    ...
    // add in any extra field we want to get
    _.each(config.extraFields, function(field) {
      _.each($(form).find(field), function(found) {
        elements.push(found);
      });
    });
  ...
  var getElementType = function(el) {
    ...
    var type = tagName || $el[0].nodeName;
  ...
  var buildConfig = function(options) {
    ...
    config.ignoredDatas = _.merge(Syphon.ignoredDatas, config.ignoredDatas);
    config.extraFields = _.merge(Syphon.extraFields, config.extraFields);
  ...
  var flattenData = function(config, data, parentKey) {
    ...
    _.each(data, function(value, keyName) {
      ...
      if (_.isArray(value)) {
        keyName += '[]';
        if (_.isArray(hash[keyName])) {
          hash[keyName].concat(value);
        } else {
          hash[keyName] = value;
        }
  ...
  KeyExtractors.register('div', function($el) {
    return $el.attr('name') || '';
  });
  ...
  InputReaders.register('div', function($el) {
    return $el.html();
  });
